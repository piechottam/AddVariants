...
public class VCFVariantWriter<T extends BaseQualRecordData> 
extends AbstractVariantFileWriter<T> {

	public static final char COMMENT ='#';
	public static final char EMPTY 	='.';
	public static final char SEP 	= '\t';

	public VCFVariantWriter(final String filename, final VCFVariantFormat<T> format) {
	...
	public void addHeader(List<Condition<T>> conditions) {
		...
		sb.append("##source=");
		sb.append(AddVariants.NAME + "-" + AddVariants.VERSION);
		sb.append('\n');
		...
		*/
		...
		for (final Condition<T> condition : conditions) {
			sb.append(condition.getRecordFilename());
		}
		addLine(sb.toString());
	}
	
	@Override
	public void addVariants(final Variant[] variants, final List<Condition<T>> conditions) throws Exception {
		final StringBuilder sb = new StringBuilder();
		...
		for (; conditionIndex < conditions.size(); conditionIndex++) {
		...
		for (conditionIndex = 0; conditionIndex < conditions.size(); conditionIndex++) {
			// TODO format
		}
